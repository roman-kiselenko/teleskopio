...
export function ClusterLink({ disabled = false }) {
  const currentPath = useLocation();
  const isClusterPage = useMemo(() => currentPath.pathname === '/cluster', [currentPath]);
  const children = (
    <div
      className={`flex items-center justify-center flex-shrink-0 w-10 h-10 mt-3 rounded hover:bg-blue-300 ${
        isClusterPage ? 'bg-blue-100' : ''
      }`}
    >
      <div
      ...
      >
        <FontAwesomeIcon icon={faDiagramProject} size="lg" className="" />
      </div>
    </div>
  );
  return (
