      ...
      return (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem
              className="text-xs"
              onClick={() => navigator.clipboard.writeText(sc.metadata.name)}
            >
            ...
            <DropdownMenuItem
              disabled={sc.metadata?.deletionTimestamp !== undefined}
              className="text-xs"
              onClick={async () => {
                toast.promise(
                  invoke<StorageClass>('delete_sc', {
                    path: getKubeconfig(),
                    context: getCluster(),
                  ...
                  }),
                  {
                    ...
                    success: () => {
                      return (
                        <span>
                          StorageClass <b>{sc.metadata.name}</b> deleted
                        </span>
                      );
                    },
                    error: (err) => (
                      <span>
                        Cant delete storageclass <b>{sc.metadata.name}</b>
                        <br />
                        {err.message}
                      </span>
                    ),
